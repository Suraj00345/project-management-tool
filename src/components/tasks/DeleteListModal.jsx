...
import { deleteListApi } from "../../utils/api-client";
import { useParams } from "react-router-dom";

const DeleteListModal = ({ listId, onDelete }) => {
  ...
  const deleteHandler = async () => {
    ...
    try {
      setIsLoading(true);
      const res = await deleteListApi(projectId, listId);
      if (!res) throw new Error("List deletion failed");
      ...
      toast.error(error.message || "Error deleting list");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <>
      <button onClick={() => setIsOpen(true)} className="text-gray-500 hover:text-red-500 p-1 rounded cursor-pointer">
        <Trash2 size={16} />
      </button>
        ...
        <div className="fixed inset-0 z-10 w-screen overflow-y-auto backdrop:blur-md ">
          <div className="flex min-h-full items-center justify-center p-4">
            ...
            >
              ...
              <p className="mt-2 text-sm/6 text-gray-600">Are you sure you want to delete this List? This action cannot be undone.</p>
