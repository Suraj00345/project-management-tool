...
const List = ({
    ...
    return (
      ...
      >
        ...
        <div className="flex-1 flex items-center justify-center">
          <h3 className="font-semibold text-gray-800 text-sm whitespace-nowrap transform -rotate-90 origin-center">
            {list.title}
          </h3>
        </div>

        {/* Card Count */}
        <div className="text-xs text-gray-500 bg-gray-200 rounded-full w-6 h-6 flex items-center justify-center">
          {list.cards.length}
        </div>
      </div>
    );
  ...
  return (
    <div
      className={`bg-gray-100 rounded-lg p-4 flex-shrink-0 w-80 h-fit ${
        isDraggedOver ? "ring-2 ring-blue-400 bg-blue-50" : ""
      }`}
      onDragOver={(e) => onDragOver(e, list.id)}
      onDragLeave={onDragLeave}
